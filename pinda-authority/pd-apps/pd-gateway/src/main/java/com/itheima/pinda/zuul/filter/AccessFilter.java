...
public class AccessFilter extends BaseFilter {
    ...
    public Object run() {
        ...
        System.out.println("enter into AccessFilter...");
        //第1步：判断当前请求uri是否需要忽略
        if (isIgnoreToken()) {
            //当前请求需要忽略，直接放行
            return null;
        ...
        long count = list.stream().filter((resource) -> permission.startsWith(resource)).count();
        ...
        // 如果没有取到则通过Feign调用权限服务获取并放入缓存，判断用户拥有的权限是否包含当前请求的权限标识符
        ...
        count = visibleResource.stream().filter((resource) -> permission.startsWith(resource)).count();
